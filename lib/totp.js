...
TOTP.prototype = {
    ...
    gen: async function(timeStep = 30, bias = 0) {
        ...
        // Check browser or nodejs w/ webcrypto
        let subtleCrypto;
        if (typeof window !== 'undefined') subtleCrypto = window.crypto.subtle;
        else if (typeof crypto.subtle !== 'undefined') subtleCrypto = crypto.subtle;
        
        if ( subtleCrypto !== 'undefined' ){
            // compute HMACSHA1(key, shift)
            const key = await subtleCrypto.importKey("raw", _keybytes,
                {name: "HMAC", hash: { name: "SHA-1" }},
                false,   // no export 
                ["sign"] // what this key can do
            );
            const signature = await subtleCrypto.sign( "HMAC", key, _timeFactor );
